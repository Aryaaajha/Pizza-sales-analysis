...
-- Example: Daily Trend for Total Orders filtered by Month (January)
SELECT DATENAME(DW, order_date) AS order_day, COUNT(DISTINCT order_id) AS total_orders FROM pizza_sales WHERE MONTH(order_date) = 1 GROUP BY DATENAME(DW, order_date);

-- Example: Daily Trend for Total Orders filtered by Quarter (First Quarter)
SELECT DATENAME(DW, order_date) AS order_day, COUNT(DISTINCT order_id) AS total_orders FROM pizza_sales WHERE DATEPART(QUARTER, order_date) = 1 GROUP BY DATENAME(DW, order_date);